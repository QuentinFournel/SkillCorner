...
PASSES_OPTIONS = {
    "Pass Opp.": ("count_opportunities_to_pass_to_runs_per_30_min_tip", "Pass Opp.", "opp/30min"),
    "Pass Att.": ("count_pass_attempts_to_runs_per_30_min_tip", "Pass Att.", "pass/30min"),
    "Pass %": ("pass_completion_ratio_to_runs", "Pass %", "%"),
    "Pass2Dang.": ("count_completed_pass_to_dangerous_runs_per_30_min_tip", "Pass2Dang.", "pass/30min"),
    "Pass2Shot": ("count_completed_pass_to_runs_leading_to_shot_per_30_min_tip", "Pass2Shot", "pass/30min"),
    "Pass2Goal": ("count_completed_pass_to_runs_leading_to_goal_per_30_min_tip", "Pass2Goal", "pass/30min"),
    "Pass Thr.": ("pass_opportunities_to_runs_threat_per_30_min_tip", "Pass Thr.", "thr/30min"),
    "Pass Thr. IB": ("pass_opportunities_to_runs_in_behind_threat_per_30_min_tip", "Pass Thr. IB", "thr/30min"),
    "Pass Att. IB": ("count_pass_attempts_to_runs_in_behind_per_30_min_tip", "Pass Att. IB", "pass/30min"),
    "Pass Cmpl. IB": ("count_completed_pass_to_runs_in_behind_per_30_min_tip", "Pass Cmpl. IB", "pass/30min"),
}
...
def load_passes_data():
    ...
    for id in COMPETITION_DICTIONARY.keys():
        data = client.get_in_possession_passes(params={
            'competition_edition': id,
            'playing_time__gte': 60,
            'count_match__gte': 8,
            'average_per': '30_min_tip',
...
@st.cache_data
def get_skillcorner_data(dataset_name, df_off_ball_runs, df_physical, df_passes):
...
def plot_radar_chart(df, player_name, selected_metrics):
    ...
    metrics_dict = {value[0]: value[1] for value in selected_metrics.values()}
...
def plot_bar(df, metric_to_use, metric_label, metric_unit):
    ...
    fig, _ = bar.plot_bar_chart(df, 
                                 ...
                                 label=metric_label,
                                 unit=metric_unit,
                                 primary_highlight_group=top_players,
                                 add_bar_values=True,
                                 data_point_id='player_id',
...
def load_skillcorner_data(df_off_ball_runs, df_physical, df_passes):
    data_choice = st.selectbox("Choose a dataset", ["Off-Ball Runs", "Physical Data", "Passes"])
    
    if data_choice == "Off-Ball Runs":
        st.session_state.df = df_off_ball_runs
        st.session_state.dataset_type = "Off-Ball Runs"
    elif data_choice == "Physical Data":
        st.session_state.df = df_physical
        st.session_state.dataset_type = "Physical Data"
    elif data_choice == "Passes":
        st.session_state.df = df_passes
        st.session_state.dataset_type = "Passes"
    
    st.success(f"✅ {data_choice} data successfully loaded!")

def st_action(df_off_ball_runs, df_physical, df_passes):
    ...
    with st.sidebar:
        ...
        if importation_system == "CSV":
            ...
            load_skillcorner_data(df_off_ball_runs, df_physical, df_passes)
    ...
    if page == "Data Overview":
        ...
        if not st.session_state.df.empty:
            ...
            if "dataset_type" in st.session_state:
                ...
                elif st.session_state.dataset_type == "Physical Data":
                    available_metrics = PHYSICAL_OPTIONS
                elif st.session_state.dataset_type == "Passes":
                    available_metrics = PASSES_OPTIONS
            else:
        ...
        if not st.session_state.df.empty:
            ...
            player_teams = st.session_state.df[st.session_state.df['player_name'] == player]['team_name'].unique()
            ...
            if len(player_teams) > 1:
                team = st.selectbox("Select a team for this player", player_teams)
            else:
                team = player_teams[0]

            # Filtrer le dataframe avec le joueur et son poste sélectionné
            player_row = st.session_state.df[(st.session_state.df['player_name'] == player) & (st.session_state.df['group'] == position) & (st.session_state.df['team_name'] == team)]
    ...
    if st.session_state.authenticated:
        with st.spinner("Loading data from SkillCorner..."):
            try:
                ...
                spinner_off_ball = st.empty()
                spinner_physical = st.empty()
                spinner_passes = st.empty()

                # Affichage du message de chargement
                spinner_off_ball.text("🔄 Loading Off-Ball Runs...")
                spinner_physical.text("🔄 Loading Physical Data...")
                spinner_passes.text("🔄 Loading Passes Data...")

                # Exécuter les appels API en parallèle
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future_off_ball = executor.submit(load_off_ball_runs_data)
                    future_physical = executor.submit(load_physical_data)
                    future_passes = executor.submit(load_passes_data)

                    # Récupérer les résultats
                    df_off_ball_runs = future_off_ball.result()
                    df_physical = future_physical.result()
                    df_passes = future_passes.result()

                # Supprimer les messages de chargement après l'exécution
                spinner_off_ball.empty()
                spinner_physical.empty()
                spinner_passes.empty()
        ...
        st_action(df_off_ball_runs, df_physical, df_passes)